...
def order_send(request, order_id):
    ...
    works = order.work.all()
    masters1 = Master.objects.annotate(count=Count('work')).filter(work__in=works, count=len(works))
    master_list = Master.objects.filter(work__in=works, city=order.city.all())
    master_list = master_list.annotate(Count('work')).filter(work__count=len(works))
